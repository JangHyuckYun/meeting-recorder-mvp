...
const PROVIDER_TYPE_LABELS: Record<string, string> = {
  openai: "OpenAI",
  anthropic: "Anthropic",
  openai_compatible: "vLLM (OpenAI 호환 형식)",
};

const PROVIDER_TYPE_OPTIONS = [
  { value: "openai", label: "OpenAI" },
  { value: "anthropic", label: "Anthropic" },
  { value: "openai_compatible", label: "vLLM (OpenAI 호환 형식)" },
];
...
function AddProviderForm({
  ...
  return (
    <div className="add-provider-form">
      ...
      <label className="settings-row">
        ...
        <select
          ...
          onChange={(e) => {
            ...
            if (e.target.value === "openai_compatible") setAuthMode("api_key");
          }}
        >
      ...
      {providerType !== "openai_compatible" && <fieldset className="settings-row">
        <legend className="settings-row-label">인증 방식</legend>
        <label><input type="radio" checked={authMode === "api_key"} onChange={() => setAuthMode("api_key")} /> API key</label>
        <label><input type="radio" checked={authMode === "oauth"} onChange={() => setAuthMode("oauth")} /> OAuth</label>
      </fieldset>}
      {authMode === "api_key" && <label className="settings-row">
      ...
      {providerType === "openai_compatible" && <p className="settings-hint">vLLM 등 OpenAI 호환 API 서버. base URL 필요</p>}
      <label className="settings-row">
...
export function SettingsModal({ open, onClose }: SettingsModalProps) {
  ...
  const [oauthPaste, setOauthPaste] = useState<Record<string, string>>({});
  ...
  const loadAppSettings = async () => {
    ...
    setSttEngine(appSettings.stt_engine ?? "elevenlabs");
    ...
    const load = async () => {
      ...
      for (const provider of loadedProviders) {
        if (!provider.is_builtin && provider.auth_mode === "oauth") {
          try { await refreshOAuthStatus(provider.provider_type === "anthropic" ? "claude_oauth" : "codex_oauth"); } catch { /* status is optional */ }
        }
      }
    };
  ...
  const oauthKey = (provider: Provider) => provider.provider_type === "anthropic" ? "claude_oauth" : "codex_oauth";
  const refreshOAuthStatus = async (key: string) => {
    const status = await appClient.getOAuthStatus(key);
    setOauthStatuses((prev) => ({ ...prev, [key]: status }));
    return status;
  };
  const handleOAuthLogin = async (provider: Provider) => {
    const key = oauthKey(provider);
    setOauthLogin(provider.id);
    setProviderError(null);
    try {
      await appClient.startOAuthLogin(provider.provider_type === "anthropic" ? "anthropic" : "openai");
      const started = Date.now();
      const poll = async () => {
        const status = await refreshOAuthStatus(key);
        if (status.logged_in || Date.now() - started >= 120_000) {
          setOauthLogin(null);
          return;
        }
        try {
          const completed = await appClient.completeOAuthLogin(provider.provider_type === "anthropic" ? "anthropic" : "openai");
          setOauthStatuses((prev) => ({ ...prev, [key]: completed }));
          setOauthLogin(null);
          return;
        } catch { /* callback not received yet */ }
  ...
  const handleOAuthPaste = async (provider: Provider) => {
    const value = oauthPaste[provider.id]?.trim();
    if (!value) return;
    const key = oauthKey(provider);
    setProviderError(null);
    setOauthLogin(provider.id);
    try {
      const status = await appClient.completeOAuthLogin(provider.provider_type === "anthropic" ? "anthropic" : "openai", value);
      setOauthStatuses((prev) => ({ ...prev, [key]: status }));
      setOauthPaste((prev) => ({ ...prev, [provider.id]: "" }));
    } catch (e) { setProviderError(errorMessage(e)); }
    finally { setOauthLogin(null); }
  };
  ...
  const handleLoadModels = async (providerId: string) => {
    ...
    try {
      ...
      if (provider && !provider.is_builtin) await appClient.updateProvider({ id: provider.id, name: provider.name, provider_type: provider.provider_type, base_url: provider.base_url, api_key: "", auth_mode: provider.auth_mode, models_json: JSON.stringify(models) });
    } catch (e) { setProviderError(errorMessage(e)); }
                ...
                {providers.length > 0 && (
                  <div className="settings-rows">
                    {providers.map((provider) => {
                      ...
                      else if (provider.auth_mode === "oauth") oauthProviderId = provider.provider_type === "anthropic" ? "claude_oauth" : "codex_oauth";
                      ...
                      return (
                        <div key={provider.id} className="provider-row">
                          <div className="entry-main">
                            ...
                            {provider.provider_type === "openai_compatible" && provider.base_url && (
                              <div className="entry-detail" data-numeric>
                                {provider.base_url}
                              </div>
                            )}
                            {provider.auth_mode === "api_key" && provider.api_key_masked && (
                            ...
                            {(provider.auth_mode === "oauth" || provider.is_builtin) && <div className="provider-oauth-actions">
                              <Button variant="outline" size="sm" onClick={() => void handleOAuthLogin(provider)} disabled={oauthLogin === provider.id}>
                                {oauthLogin === provider.id ? "브라우저에서 로그인 중…" : "로그인"}
                              </Button>
                              <input className="settings-input" value={oauthPaste[provider.id] ?? ""} onChange={(e) => setOauthPaste((prev) => ({ ...prev, [provider.id]: e.target.value }))} placeholder="코드/리다이렉트 URL 붙여넣기" aria-label="OAuth 코드 또는 리다이렉트 URL" />
                              <Button variant="ghost" size="sm" onClick={() => void handleOAuthPaste(provider)} disabled={oauthLogin === provider.id}>확인</Button>
                            </div>}
                          </div>
                          {!provider.is_builtin && (
            ...
            <TabsContent value="voice" className="settings-tab-panel">
              <section className="settings-section">
                ...
                <div className="settings-rows" data-testid="stt-engine-select">
                  ...
                  {sttEngine === "self_hosted" && <label className="settings-row">
                    <span className="settings-row-label">
                      서버 URL
